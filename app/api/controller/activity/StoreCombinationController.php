...
class StoreCombinationController
{
    ...
    public function pink(Request $request, $id)
    {
        ...
        if($pink['stop_time'] < time()){
            return app('json')->fail('拼团已结束');
        }
        //验证团内人数
        $cache_pink = Cache::get(md5('store_pink_'.$pink['id']));
        if(bcsub($pink['people'], $cache_pink['now_people'], 0) <= 0){
            return app('json')->fail('该团已满员，请去开团邀请好友加入');
        }
